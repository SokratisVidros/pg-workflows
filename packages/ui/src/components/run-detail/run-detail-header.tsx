...
export function RunDetailHeader({ run, onBack, className }: RunDetailHeaderProps) {
  const duration = computeDurationMs(run);
  const resourceId = (run as unknown as { resourceId?: string }).resourceId;
  return (
    <header className={cn('flex flex-col gap-2 border-b border-pgw-border pb-4', className)}>
      {onBack && (
        <button
          type="button"
          onClick={onBack}
